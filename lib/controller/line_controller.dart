...
class LineController {
  ...
  void init() {
    ...
    _timer = Timer.periodic(const Duration(milliseconds: 100), _timerCallback);
  }
  ...
  void _handleSensorData(Vector2 vector2) async {
    if (_connected && _send) {
      if (vector2.x < 1) {
        ...
        if (_left) {
          _bluetoothController.write(
              type: ControllerType.left, value: vector2.x.toInt());
        }
        if (!_left) {
          _bluetoothController.write(
              type: ControllerType.right, value: vector2.x.toInt());
        }
      }
